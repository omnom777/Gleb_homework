...
class Diary:
    ...
    def add_grade(self, subject, grade):
        if not isinstance(subject, str):
            print("Ошибка: предмет должен быть строкой")
            return
        if not subject.strip():
            print("Ошибка: название предмета не может быть пустым")
            return
        if not isinstance(grade, (int, float)): #isinstance() - проверяет, принадлежит ли объект определённому классу или типу данных
    ...
    def get_bad_subjects(self):
        ...
        for i in range(len(self._subjects)):
            ...
            vsego = 0
            count = 0
            for j in range(len(self._subjects)):
                if self._subjects[j] == subject:
                    vsego += self._grades[j]
                    count += 1
